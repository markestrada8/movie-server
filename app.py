...
class Movie(db.Model):
    ...
    all_reviews = db.relationship(
        'Review', backref='movie', cascade='all,delete,delete-orphan')
...
class MovieSchema(ma.Schema):
    class Meta:
        fields = ('id', 'title', 'genre', 'mpaa_rating',
                  'poster_image', 'all_reviews')
...
def movie_to_edit(id):
    ...
    if poster_image != None:
        movie_to_edit.poster_image = poster_image
...
def add_many_movies():
    ...
    for movie in post_data:
        ...
        existing_movie_check = db.session.query(
            Movie).filter(Movie.title == title).first()
